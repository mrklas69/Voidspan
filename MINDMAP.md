...
Verze: **v1.6** (2026-04-13, konec sezení 10 — UI kostra §16 živá: HUD/Actors/Segment 8×2/TaskQueue/Inspector/Log, model-driven, bez dekoračních rámečků. `types.ts` → `model.ts` s katalogy MODULE_DEFS (P1: SolarArray/Engine/Dock), ACTOR_DEFS, TASK_DEFS. Vitest 2.1.9 + 31 testů zelených (model-first). Asset pipeline: source 40×40 v `art/` s magenta chroma key → `pnpm build:assets` → ship do `public/assets/`. `ArtRefScene` odstraněna (img2img workflow nahrazen hand-pixel). První asset `floor.png` (empty tile).)
...
│   ├── 6.4 Art pipeline (hand-pixel 40×40, chroma key) [◐] S10 pipeline live
│   └── 6.5 Moderation & LLM                           [○]
│
...
**UI kostra + asset pipeline živé (S10).** `model.ts` s katalogem, `world.ts` čistě funkční, `GameScene` renderuje z modelu: HUD / Actors / Segment 8×2 / TaskQueue / Inspector / Log bez dekoračních rámečků, jen sprity. Vitest 31 testů zelených (FSM + catalog invariants). Asset pipeline: `art/<kat>/<name>.png` (magenta chroma key) → `pnpm build:assets` → ship do `public/assets/`. První asset `floor.png` pro empty tile. Klik na tile → Inspector detail z `MODULE_DEFS`.

**Další bagr:**
1. **Task engine + input §15** — první interaktivní cyklus (klik damaged → task → assign → WD progress).
2. **Assety P1** — nakreslit SolarArray, Engine, Dock, damaged tile (hand-pixel 40×40 magenta).
3. **Push na GitHub + GH Pages deploy + CI** (GH Actions s `pnpm test`) — sdílitelná URL.
4. **Playtest kalibrace CAL-*** — seed hodnoty → ladění po prvním hratelném buildu.
5. **3.2 Postava hráče** — brains UX, Capability Matrix (dokumentačně pro P2+).
6. **4.2 Colony Arc refactor** — přerovnat legacy Act -1 až Post-Closure.
