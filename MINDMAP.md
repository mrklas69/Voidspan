...
Verze: **v3.7** (2026-04-17, konec sezení 29 — **Font swap VT323 → Atkinson Hyperlegible** (sans, accessibility-grade, plná CS diakritika). Přechod stroj hejna → mrkla (discard WIP palette.ts, pull 5 commitů S25-S28). **Welcome reset přes Help [H]** — modal.action API rozšířeno o volitelné druhé tlačítko („Zobrazit uvítání"), `welcome.resetWelcome()` export, `GameScene.openWelcomeDialog()` helper s lazy create. **DSO 1/8** — `DSO_CHANCE` 0.35 → 0.35/8 (mlhoviny jako vzácné výjevy). **Font preview tooling** — `public/font-preview.html` s 7 kandidáty (5 mono + 2 sans) ve 3 velikostech. **Globální bump -2 px** — CHROME 24→22, PANEL 22→20, TIP 20→18 (sans má větší x-height). **Nová konstanta `FONT_SIZE_SIDEPANEL = 18px`** — extra -2 px pro I/M/E/T panely (hustota dat ≈ tooltip); modal/welcome/commandButton/ActorsPanel ponechány na PANEL=20. 88/88 testů zelených.)
