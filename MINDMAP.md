...
**S23 hotovo:** Tooltip barevný header (TooltipContent, headerText, 5stavový semafor). SIGN eventy barevné (RATING_COLOR). Phase retirement (phase_a/b/c → boot|running). Kvintet infotipy E/W/S/F/C (barevný header, Kapacita/Příjmy/Výdaje/Bilance). ENERGY_MAX dynamický (World.energyMax). Work model eureka (E↔W symetrie, hráč HP+work, dron=číslo převodník E→WD). Hauler retirement.

**S22 hotovo:** Mobile touch. Event format axiom CZ. InfoPanel UX. Energy infotip. Tooltip responzivní šířka.

**Další bagr:**
1. **Energy=0 → freeze + cryo failure** — všechny entity bez E se zastaví, HP astronautů klesá. Řetěz zánik.
2. **Per-capita resource drain** — `resourceDrain` stub → `n_alive × per_actor_rate`.
3. **autoEnqueueTasks** — drony samy opravují critical HP. Observer-driven priority queue.
4. **Práce spotřebovává HP hráče** — progressTasks odečítá HP working aktérům.
