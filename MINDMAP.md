...
Verze: **v1.4** (2026-04-13, konec sezení 8 — skeleton živý: pnpm monorepo + Vite + Phaser 3.87 + TS strict, `BootScene` renderuje 2×8 reference grid s PNG exportem, img2img workflow ověřen jako style placeholder)
...
**Skeleton živý (S8).** pnpm monorepo + Vite + Phaser 3.87 + TS strict běží lokálně. `BootScene` renderuje 2×8 reference grid + PNG export. Img2img workflow s referencí ověřen — dodrží geometrii tiles, ale zaoblený obrys lodi model přepisuje. Přijato jako **style placeholder**.

**Další bagr:**
1. **Art sezení P1** — katalog promptů pro všechny tiles/aktéry/efekty, finální styl lock. Img2img s grid referencí.
2. **Data model + FSM do kódu** — převést POC_P1 §13–14 do `apps/client/src/` (World, Tile, Module, tick 250 ms, phase_a/b/c).
3. **UI kostra dle §16** — HUD/Actors/Segment/TaskQueue/Log zóny jako prázdné bloky před naplněním obsahem.
4. **Push na GitHub + GH Pages deploy** — sdílitelná URL.
