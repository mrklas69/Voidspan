...
**Příští sezení (S38) — v1.1 kosmetika + R2 prep:**
- **Damage particles burst** (v1.1 — odloženo z S37 plánu, TODO S35 follow-up) — Phaser.Particles při asteroid hit + při completion. Orange sparks, 2s burst, ~30 částic. ~40 LOC, GPU cost OK pro single-burst.
- **Sanity playtest cestovního layoutu** — ShipRender + deterministická stavebnice + 4 void na čele. Ověřit selection WASD po 4 voidech, tooltip „Void — prázdný slot, lze stavět (P2+)", vizuál cestovní minimalist sedí.
- **Wake-up mechanismus (R2 start)** — první kolonista se probouzí (welcome CTA „Probudit" nebo auto trigger). Cryo → idle → working lifecycle.
- **Module research & upgrade paths (IDEAS S36)** — Solar 1×1 → 2×2 (10× produkce). Q1-Q5 decision (rebuild vs. in-place / cost / prereq / reverz / visual).
