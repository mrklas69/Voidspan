...
Verze: **v4.9** (2026-04-19, konec sezení 41 — **Osa 2 etapa 4: klient server-mode + protocol v2 + AppName infotip polish**). Hlavní změny:
- **Schema v1 → v2** — `Event.id: number` monotónní counter + `World.nextEventId` v `packages/shared/src/model.ts`. `appendEvent` inkrementuje, `Omit<Event, "id"|"tick"|"verb"|"severity">` rozšířeno. Umožňuje klientovi deduplikovat eventy po reconnectu (HELLO recentEvents × EVENT stream se překryjí). `SCHEMA_VERSION = 2` v protocol.ts.
- **Q5 nuke & restart ověřen** — server boot s old `world.json` (v1) → `[persistence] schema mismatch (soubor v1, kód v2) — nuke & restart` + `[boot] fresh world vytvořen`. Mechanika z Ose 2 Q5 rozhodnutí funguje bez zásahu.
- **`apps/client/src/net/ws_client.ts`** (~180 LOC, nový) — tenký WS wrapper. `WsClient` třída + `WsClientHandlers { onHello, onSnapshot, onEvent, onStatus }` + `WsStatus` union. Exp backoff 1s → 2s → 4s → ... → cap 30s + jitter ±20 % (reconnect storm protection). PING heartbeat 10 s (server timeout 30 s margin). Backoff reset až po HELLO (ne po open). `closedByUser` flag blokuje auto-reconnect po manual close. `onError` jen logging — reconnect plánuje až `onClose` (jinak 2× call). PONG reply tiše drop.
- **Feature flag `?server=ws://host:port`** v GameScene. `serverMode: boolean` + `serverUrl: string|null` + `wsStatus: WsStatus` + `wsClient: WsClient|null`. Placeholder `createInitialWorld()` vytvořen vždy (panely čtou ihned), HELLO přepíše `this.world` referencí (panely drží `getWorld = () => this.world` lazy closure). `update()`: `if (!serverMode)` skip stepWorld. onEvent dedup: `if (events.last().id >= ev.id) return` + ring buffer splice (EVENT_LOG_CAPACITY).
- **AppName infotip polish** (user request „že server není standalone by se mohlo zobrazit i v infotipu na AppName") — `ServerInfo` type exportován z GameScene, `getServerInfo: () => ServerInfo` closure předán do HeaderPanel 4. constructor parametrem (optional, backward compat). Infotip na brand/AppName/meta: `Runtime: local standalone (klient stepuje sám)` nebo `Runtime: server (ws://localhost:3000) — connected`. speedSuffix `×N` skryt v server mode (server běží pevně ×1), popover click no-op (`return` před toggle).
- **E2E ověřeno** — smoke client: `HELLO schema=v2 tick=69 events=1`, `PONG`, `SNAPSHOT #1/#2 tick=70/80`. Node jednorázový check: `{"id":1,"tick":0,"verb":"SYST","text":"Simulace spuštěna"}`, `nextEventId: 2`. Browser E2E uživatelsky potvrzený: `http://localhost:5173/?server=ws://localhost:3000` zobrazuje serverový stav, time speed neaktivní jak se čeká.
- **Testy beze změny** — 183/183 zelených (shared 149 + client 34). TS strict clean napříč 3 balíčky.
- **Scope diff vs. původní Etapa 3 plán:** Etapa 3 (delta broadcasts + reconnect UX) posunuta NA DRUHÝ směr — design without reálného klienta by byl slepý, s fungujícím klientem teď uvidíme reálný bandwidth (dnes full SNAPSHOT ~50 KB × 0.4/s = 20 KB/s, pro 1-5 FVP klientů OK). WS status dot v Top Baru + DecisionModal gate zůstávají jako Etapa 3 scope.
