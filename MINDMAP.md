...
**Příští sezení (S35) — Procedural ship render (po Gemini style extraction):**
- Implementovat `apps/client/src/game/ui/ship_render.ts` (nebo rozšířit `segment.ts`) procedurálně: Phaser.Graphics base layer (rounded rect outline per modul kind, palette color), Phaser.FX.Glow při select/damage/completion, per-modul funkční glyph (8 ikon přes `lineTo()`, ~20-40 bodů každý), status izomorfismus (ONline solid / DAMAGED yellow dashed + zig-zag / OFFLINE grey + checkered / staví se progress bar), void „+" placeholder.
- Particle damage overlay (Phaser.GameObjects.Particles) — oranžové sparks 2s burst při `flashUntilTick`, existující infrastruktura.
- **Stavebnice (1×2, 2×2, 1×3, 1×4)** — Gemini nedodalo, musíme rozhodnout sami. Návrh v IDEAS Q2: Engine 1×2, SolarArray 1×3, CommandPost 2×2, MedCore 1×2, ostatní 1×1. Update `MODULE_DEFS.w/h` v `model.ts`, fallback v `createInitialWorld` BODY_IDXS.
- Paleta revize: per-modul hue (8 barev) + OK/warn/crit (green/yellow/red). Rozhodnout: nová `palette_v2.ts` nebo přepis stávající `palette.ts`. Amber jako UI text zachovat nebo také posunout?
- LOC cíl ≤500 pro celou visual vrstvu. `art/` adresář = moodboard + `arena_winners/` (žádné per-entity PNG).

**Další bagr po S35 (R2 Player Awakening prep):**
- Wake-up mechanismus, Player HP ekonomika, Commands palette, Scripted capsule drops — viz „Další bagr" sekce níže.
