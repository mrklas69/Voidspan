...
Verze: **v1.3** (2026-04-13, konec sezení 7 — POC_P1 v0.7, spec ready-to-code: §12 pure client, §13–15 data+FSM+input, §16 UI wireframe, §17 onboarding, §18 závěrečné dialogy)
...
Body 1, 3.1, Q17 uzavřeny. **POC_P1 v0.7 je spec ready-to-code** (§1–18): scope, kalibrace seed, architektura pure-client, data model + state machine + input model, UI wireframe, onboarding, závěrečné dialogy. Poslední blokátor vizuální kvality = Art sezení.

**Další bagr:**
1. **Art sezení P1** — asset pipeline dle `POC_P1.md §9+§16` (tile sprites, aktéři, Tabler ikony, finální pass). Samostatné sezení.
2. **`pnpm init` + Vite skeleton** — první commit kódu, Hello World v Phaseru.
3. **Playtest kalibrace CAL-*** — seed hodnoty → ladění po prvním funkčním buildu.
4. **3.2 Postava hráče** — brains UX, Capability Matrix (dokumentačně pro P2+).
5. **4.2 Colony Arc refactor** — přerovnat legacy Act -1 až Post-Closure.
