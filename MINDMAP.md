...
Verze: **v1.2** (2026-04-12, konec sezení 6 — Q17 uzavřeno: POC_P1 single-player puzzle, SHIP revize na 1 segment, `POC_P1.md` založen)
...
Body 1, 3.1, **Q17 uzavřeny**. POC_P1 = single-player puzzle s WIN/LOSS (viz `POC_P1.md`). SHIP revidován na 1 segment (S5 2-segment config byl over-provisioned).

**Další bagr:**
1. **Art sezení P1** — asset pipeline (tile sprites, VFX, UI) dle `POC_P1.md §9`. Samostatné sezení.
2. **Kalibrace CAL-*** — WD náklady, timeout, time compression, solar power po shrinku. Ladí se playtestem.
3. **3.2 Postava hráče** — brains schedule UX (Q-Brains-Schema), Capability Matrix, housing/HP.
4. **4.2 Colony Arc refactor** — přerovnat legacy Act -1 až Post-Closure.
5. **3.3 Oblasti hry** — 3.3.1 Materiál & provoz, 3.3.3 Řád.
