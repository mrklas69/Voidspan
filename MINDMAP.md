...
Verze: **v1.1** (2026-04-12, konec sezení 5 — 3.1 Prostor a čas uzavřena; datový model WORLD/BELT/SEGMENT/MODULE/TILE, Energy Model, Capability Matrix, SHIP config, Teegarden System)
...
│   │     Energy Model W/WD, 16h den, TIME_COMPRESSION 16×)
│   ├── 3.2 Postava hráče                              [◐]
...
│   ├── 6.1 Stack (Node + TS + Colyseus + Phaser)      [●]
│   ├── 6.2 Hosting (Render → VPS)                     [●]
│   ├── 6.3 Monorepo (pnpm workspace)                  [◐]
│   ├── 6.4 Art pipeline (8-bit, Phaser animace)       [○]
│   └── 6.5 Moderation & LLM                           [○]
│
...
Body 1 + 3.1 **uzavřeny**. 3.1 přinesla kompletní datový model (hierarchie, SHIP, Energy Model, Capability Matrix, Drone Fleet, Teegarden System).

**Další bagr:**
1. **3.2 Postava hráče** — brains schedule UX (Q-Brains-Schema), Capability Matrix kalibrace, housing state, HP drain.
2. **3.3 Oblasti hry** — rozepsat 3.3.1 Materiál & provoz (dopady Energy Modelu na ekonomiku), 3.3.3 Řád (lawlessness, justice).
3. **Q17 rozsah P1 POC** — definovat minimální hratelný sim (navrhováno: SHIP wake-up + první 3-4 hráči + postavit Greenhouse + demontovat Engine → Dock).
4. **4.2 Colony Arc refactor** — přerovnat legacy Act -1 až Post-Closure.

**Terminologie prototypů:** P1/P2/P3/P4 = **POC** (proof of concept), ne MVP. Cíl = dialog s blízkými, ne trh.

**Terminologie entit:** WORLD → BELT → SEGMENT → MODULE → TILE. Pojem „Cell" je retirován, v nových textech nepoužívat.
