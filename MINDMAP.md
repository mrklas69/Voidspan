...
**Příští sezení (S37) — sanity playtest + R2 Player Awakening prep:**
- **Sanity playtest cestovního layoutu** — ShipRender + deterministická stavebnice + 4 void na čele. Ověřit: selection WASD po 4 voidech, tooltip „Void — prázdný slot, lze stavět (P2+)" jasný signál pro user, vizuál cestovní minimalist sedí (backup systémy nahoře, utility dole, expansion space na čele).
- **Wake-up mechanismus (R2 start)** — první kolonista se probouzí (welcome CTA „Probudit" nebo auto trigger). Cryo → idle → working lifecycle. Bez něj observer sim stagnuje.
- **Module research & upgrade paths (IDEAS S36)** — pokud je čas, první iterace Solar 1×1 → 2×2 upgrade mechanismu. Q1-Q5 rozhodnout: rebuild vs. in-place / cost model / prereq / reverz / visual. Vazba na 4 void na čele (tam se upgrady materializují).
- **Damage particles burst** (S35 follow-up v TODO) — Phaser.Particles při asteroid hit + při completion.

**Další bagr po S37 (R2 Player Awakening plné):**
